...
if "bpy" in locals():
    ...
    # costume / garmentcode はサブパッケージなので importlib.reload では
    # 中身が読み直されない。自前の reload_all() を operators / panels より先に呼ぶ
    # (依存の向きがこの順)。
    costume.reload_all()
    ...
    from . import costume
    from . import garmentcode
    from . import operators
    from . import panels

import bpy  # noqa: E402

# costume / garmentcode は register/unregister を持たない(UIクラスが無い)ので
# _modules に入れない。入れると有効化時に AttributeError になる。
_modules = (
    operators,
    panels,
)
