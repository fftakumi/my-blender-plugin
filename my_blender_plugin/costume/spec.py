...
_BODICE = {
    ...
    # ここはその上に足す量なので、実物の胸の高さ(4-5cm)を入れると二重計上になり、
    # 横から見て円錐のようにとがる(スク水で 51 度の稜線ができた)。
    # 「楕円だけだと平ら」を崩す程度の量にすること。スク水は 0.012 = 1.9cm
    # (稜線 37 度)。**既定の 0.022 は同じ理由で立ちすぎている**
    # (ブラウス/ベスト 58 度・ワンピース 71 度)が、既存の見た目を変えるので
    # 既定は据え置いた
