...
_WAISTBAND = {
    "waist_z": (float, 0.62, 0.0, 1.0),
    # 下端が自然なウエストからどれだけ下がっているか /H(pants と同じ意味)。
    # 接合する本体と**同じ値**にしないと、リング共有の接合がずれる
    "waist_drop": (float, 0.0, 0.0, 0.3),
...
_BODICE = {
    ...
    # 前の始末。open = 前が開いた羽織りもの(ブラウス・ベスト)。
    # closed = 前中心まで布が回った**かぶりもの**(水着・タンクトップ)。
    # closed では front_gap を使わず、前立て・ボタンを付ける相手にもならない
    "front_style": (str, "open", None, None),
    # 袖ぐりの深さ / サイズ表の袖ぐり深さ。袖を付けない衣装(水着・タンクトップ)は
    # 袖ぐりが深く大きく開く。**設計値も一緒に動く**ので袖ぐりゲートは逃げない
    "armhole_depth_scale": (float, 1.0, 0.3, 3.0),
    # 肩線の幅 / サイズ表の肩幅。1.0 は袖を付ける衣装の肩先まで。水着の肩紐は
    # 肩先まで来ないので 1 未満にする(大きいと袖ぐりの上端が肩先まで開く)。
    # armhole_depth_scale と同じく**設計値も一緒に動く**ので肩幅ゲートは逃げない
    "shoulder_width_scale": (float, 1.0, 0.5, 1.5),
    ...
    #
    # **断面の楕円はバスト周(83cm)から作っているので、胸のふくらみはもう入っている。**
    # ここはその上に足す量なので、実物の胸の高さ(4-5cm)をそのまま入れると
    # 二重計上になり、横から見て円錐のようにとがる。
    # 「楕円だけだと平ら」を崩す程度の量にすること。
    #
    # とがり具合は `validate.bust_ridge_degrees`(頂点直上の傾き)で測れる。
    # 既定 0.022 のブラウス/ベストは 24.8 度で問題ない。**丈の短い胴では
    # 同じ量でも立つ**ので、そちらは bust_t で頂点を下げて逃がす
    # (ワンピースは bust_t 0.28 で 68.7 度だったのを 0.34 = 26.3 度にした)
    "bust_projection": (float, 0.022, 0.0, 0.12),
    "bust_angular_degrees": (float, 62.0, 5.0, 175.0),  # 前中心から左右にこの角度まで
    # 前中心から左右の頂点までの角度。**0 にすると山が前中心で1つに重なり、
    # 胸骨のところがいちばん前へ出た1枚のドームになる**(実物はそこが窪むので
    # 不自然に見える)。22度 は設計値(公表寸法は見つからない)
    "bust_apex_degrees": (float, 22.0, 0.0, 80.0),
    ...
    # 袖ぐりの角の丸め。0 = 面を抜いたままの長方形、1 = 完全な楕円。
    # 実物の袖ぐりは丸いので既定は丸める(境界頂点を楕円へ載せる。辺の中点は動かない
    # ので肩幅の実測点は保たれる)
    "armhole_round": (float, 1.0, 0.0, 1.0),
}
...
#: 前スカート(旧スクの前垂れ)。腰から**前面だけ**を覆って垂れる一枚布。
#: ケープと違って前中心に布が来る(ケープは前が開いて後ろに布が来る)。
#: 数値はすべて設計値
_APRON = {
    "waist_z": (float, 0.62, 0.0, 1.0),  # 上端の高さ /H
    "length": (float, 0.11, 0.01, 0.5),  # 丈 /H
    # 前中心を中心とした角度幅。180 で前半分、360 は不可(それは skirt_body)
    "span_degrees": (float, 170.0, 20.0, 340.0),
    "segments": (int, 14, 3, 256),  # 弧の分割数(頂点数。面は1つ少ない)
    "rings": (int, 5, 2, 64),
    "flare": (float, 1.12, 0.8, 4.0),  # 裾の周長 / 上端の周長
    "flare_curve": (float, 1.2, 0.2, 6.0),
    # 本体からどれだけ外へ浮かせるか /H。下に着ている水着の面と重ならないため
    "standoff": (float, 0.005, 0.001, 0.05),
}

#: ゼッケン(名札)。胴の前面に貼る一枚の布。前立てと同じく相手の前面プロファイルに
#: 沿わせるので attach_to が必須。スク水の白い名札がこれ
_PATCH = {
    "width": (float, 0.105, 0.01, 0.4),  # 幅 /H。0.105 = 16.6cm(設計値)
    # 丈 /H。0.070 = 11.1cm(設計値)。**指定どおりにはならない** —
    # 上端・下端とも相手の行に吸着させるので、いちばん近い行までの丈になる
    "height": (float, 0.070, 0.01, 0.4),
    # 上端の位置。相手の前面プロファイルの z 幅に対する上からの比(これも行に吸着する)
    "top_inset": (float, 0.30, 0.0, 0.95),
    # 胴の面からどれだけ前へ出すか /H。**弦のたるみより大きくすること**
    # (幅方向を直線で刻むので、分割の間で布が胴の中へ入る)
    "standoff": (float, 0.0012, 0.0002, 0.02),
    "columns": (int, 6, 3, 32),  # 幅方向の分割数
    # 丈方向の最小分割数(相手の刻みが粗いときに補う)。1 = 相手の行のまま刻まない
    "rows": (int, 4, 1, 32),
}
...
_PANTS = {
    ...
    # 上端が自然なウエストからどれだけ下がっているか /H。腰穿き(ローライズ)の
    # 衣装で使う。**waist_z と足して 0.62(自然なウエストの高さ)になるように書く。**
    # 0 のままだと上端をどこに置いてもウエスト周で作られ、腰では細すぎるうえ
    # 体に沿わない垂直な筒が上端に付く
    "waist_drop": (float, 0.0, 0.0, 0.3),
    ...
    # 股下 / サイズ表の股下(0.5 でハーフパンツ)。下限 0.03 は**水着の脚口**
    # (股から数cmで終わる帯)を作れるようにしたもの
    "inseam_scale": (float, 1.0, 0.03, 1.5),
    ...
    # 脚ぐりの立ち上がり / 股上。0 = 水平に切った脚口(ズボン・スク水)。
    # >0 にすると脚ぐりが脇へ向かって上がり、ビキニ型のショーツになる。
    # 上限 0.9 は「マチだけ残して腰まで切り上げる」手前(1.0 にすると脇の布が消える)
    "leg_line": (float, 0.0, 0.0, 0.9),
    # 尻の張り出し。後ろ中心の厚みを何割増しにするか(0.25 = 25%増し)。
    # 0 なら断面は前後対称の楕円のまま(従来のズボン)
    "seat": (float, 0.0, 0.0, 0.6),
...
_TOP_LEVEL_DEFAULTS = {
    ...
    # 布の厚み /H。0.0008 = 1.3cm... ではなく 1.3mm(水着のニット相当の設計値)。
    # 0 にすると厚みゼロの一枚面になり、襟ぐりや裾の裁ち端が紙のように見える
    "fabric_thickness": (float, 0.0008, 0.0, 0.02),
    # 仕上げの細分レベル(Subsurf)。非破壊のモディファイアとして積むだけ。
    # **既定は 0(掛けない)。** 粗い格子に掛けると制御点の内側へ縮んで
    # ウエストの締まりが消え、全体が膨れる(スク水で実証)。滑らかさは
    # 素の分割数で出すのが正しく、これは意図して使うときだけのつまみ
    "subdivision": (int, 0, 0, 3),
    # joints で繋がったパーツを1つのメッシュに統合し、継ぎ目の頂点を溶接するか。
    # **既定は False。** 別オブジェクトのままだと Blender は法線を繋がないので、
    # 面が連続していても継ぎ目に陰影の段(布の切れ目に見える線)が出る。
    # True にすると線は消えるが、出力オブジェクトが減りパーツ単位の編集ができなくなる
    "join_seams": (bool, False, None, None),
}
...
_ENUM_FIELDS = {
    "side": ("l", "r"),
    "hem_style": ("shirttail", "flat"),
    "front_style": ("open", "closed"),
}
...
def json_schema_hint():
    ...
    lines = [
        ...
        # 最上位キーも**定数から生成する**。ここを手書きにしていたので
        # fabric_thickness / subdivision を足したときに hint から漏れた
        " %s,"
        % ", ".join(
            '"%s": <%s>' % (field, _TOP_LEVEL_DEFAULTS[field][0].__name__)
            for field in sorted(_TOP_LEVEL_DEFAULTS)
            if field != "assumed_height"
        ),
        ' "materials": {"<キー>": {%s}},'
        % ", ".join(
