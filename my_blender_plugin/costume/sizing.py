...
RATIO_DEFAULTS = {
    ...
    "head_height": 0.14,
    # ---- パンツ用。**すべて設計値**(出どころのある寸法から製図の定石で導出。
    # 式と根拠は docs/garments.md のパンツの節。marea の表を転記したら差し替える)
    "rise": 0.16,  # 股上(ウエスト〜股の垂直距離)≒ ヒップ/4 + 2.5cm
    "inseam": 0.43,  # 股下(股〜裾)≒ フルレングス総丈 0.59H − 股上
    "thigh": 0.34,  # わたり(太もも周)≒ ヒップ × 0.59
    "knee": 0.235,  # 膝周 ≒ ヒップ × 0.41
    "hem_opening": 0.253,  # 裾周 = スラックス標準裾幅 20cm(flat)× 2
}

#: 自然なウエストの高さ /H(**体の定数**。設計値)。衣装の waist_z の既定と同じ値だが
#: 意味が違う — waist_z は「衣装の上端をどこに置くか」で自由に動かせる。こちらは
#: 「体のウエストがどこにあるか」で、検証(validate の body_clearance)が
#: 素体の周長を高さから引くときの基準になる(SLEEVE_ELBOW_T と同じ理屈で
#: spec には持たせない — 動かせると「体を細くしてゲートを通す」が出来てしまう)
NATURAL_WAIST_Z = 0.62
